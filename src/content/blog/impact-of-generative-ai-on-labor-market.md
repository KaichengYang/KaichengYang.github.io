...
So I started looking into research on the impact of Generative AI on the job market.
There's some early evidence that AI is affecting different professions.
However, this is not what I want to focus on now.
Instead, I want to discuss a framework to help us understand the impact of AI on different tasks and jobs.

## How hard is it to verify AI output?

At least in the knowledge work realm, if you ask, "can AI do it?" The answer is almost always yes given today's state of the art.
So the right question is "can AI do it right?" or "can you tell if AI did it right?"
...
Verification can also be done automatically, and this is where AI shines.
For instance, verification is relatively objective in software engineering, and it explains why coding agents are the most developed use case so far.

But things start to get tricky when verification requires human judgement, which requires what [Zhang (2026)](#ref-zhang-2026) describes as the tacit knowledge, or taste.
...
This gives us a three-tier structure:
...
2. **At the verification threshold.** Output looks plausible but may not be correct. This is the dangerous middle zone---[Dell'Acqua et al.'s (2023)](#ref-dellacqua-2023) "jagged technological frontier," where AI improved consultant performance on tasks inside the frontier but *decreased* it outside because people over-relied on the output. The bottleneck is verification, and tacit knowledge is what makes verification possible.
...
Verification tractability is one dimension of a task.
But there's an independent one these frameworks miss: does the task's value depend on a human having performed it?

Some tasks are *output-defined*---only the result matters.
Nobody cares whether a human or an AI designed the optimal supply chain routing, as long as it works.
Other tasks are *process-defined*---who does it and how is intrinsic to the value.
Take recommendation letters as an example.
...
I think of this as *human constitutiveness*: the degree to which a task's value is constituted by human performance, not merely produced by it.
Crossing it with verification tractability gives a framework where all the cells are genuinely populated:
...
| **Criteria undefined** | Paradigm-shifting theory, novel research questions---only experts can articulate and determine what "good" means | Moral judgment, bearing witness, accountability in life-or-death decisions---a judge sentencing someone, a commander ordering troops into danger |
...
When AI absorbs the output-defined work, what remains---and what commands a premium---is the part where human performance *is* the product.
...
Their key concept is *bundle strength*---the coordination cost of unbundling.
...
Consider performing a literature review.
...
One is the evolving capabilities of AI models and agents.
The other is the trend of professionals drafting skills that distill their tacit knowledge into written guidelines and pipelines.
...
- **Cultivate the human-constitutive parts of your role.** Relationships, accountability, trust, presence. These are not soft skills to be dismissed. They're the hard floor of human value.
- **Stay adaptable.** The task you do today may shift tiers tomorrow. The meta-skill is recognizing when it does, and being willing to move rather than defending territory AI has already taken.
...
- <a id="ref-dellacqua-2023"></a>[Navigating the Jagged Technological Frontier](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4573321) --- Dell'Acqua et al. (2023)
