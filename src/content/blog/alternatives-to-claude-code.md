...
But Anthropic keeps reducing the credits allocated to subscriptions, making it harder and harder to rely on Claude Code for large amounts of work every day.
...
- **ChatGPT subscription** --- you can finish 3--6x more tasks than the Anthropic subscriptions for the same price. Note that this might change, since OpenAI is apparently still subsidizing subscriptions to gain market share.
...
And the combination of Claude Code with Claude models is even better.

However, Claude Code is not actually tied to Claude models, and you can swap in others.
In fact, models like Kimi have been optimized specifically for Claude Code.
...
It has a nice TUI and almost all the features you'd expect from Claude Code and Codex.
...
If you can afford it, Claude Code with the Max subscription is a safe choice.
The Pro subscription is barely usable these days.

A nice alternative is ChatGPT Plus/Pro.
Even the Plus tier lets you do some serious work.
Personally, I prefer Pi-agent with GPT-5.5 now.

If $20/month is still too much, consider exploring some of the open-weight models and plans.
You can still use them in a good harness.

Personally, I'm using different harnesses and models at the same time.
I have a [post](https://www.kaichengyang.me/blog/sharing-config-between-claude-code-and-codex) on how to sync the configuration and skills across different harnesses.

If your configuration depends on hooks, things get trickier though.